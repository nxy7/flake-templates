...
import { hrefForLocale, stripLocale } from "./i18n";
...
describe("komunikaty i18n", () => {
  test("angielski jest bazą, każdy język ma dokładnie te same klucze", () => {
    ...
    for (const file of files) {
      expect(Object.keys(load(file)).sort(), file).toEqual(Object.keys(base).sort());
    }
  });
...
describe("ścieżki językowe", () => {
  test("stripLocale usuwa prefiks tylko dla języków innych niż bazowy", () => {
    expect(stripLocale("/pl/app")).toBe("/app");
    expect(stripLocale("/pl")).toBe("/");
    expect(stripLocale("/about")).toBe("/about");
    expect(stripLocale("/plan")).toBe("/plan");
  });

  test("hrefForLocale buduje tę samą stronę w innym języku", () => {
    expect(hrefForLocale("/about", "pl")).toBe("/pl/about");
    expect(hrefForLocale("/pl/about", "en")).toBe("/about");
    expect(hrefForLocale("/", "pl")).toBe("/pl/");
  });
});
